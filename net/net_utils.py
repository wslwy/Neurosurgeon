...
def send_data(conn, x, msg="msg", show=True, DBG=False):
    """
    ...
    if resp_len == 'not receive':   # 超时重传一次
        conn.sendall(pickle.dumps(len(send_x)))
...
#     print("ready to send inter tensor")
#     send_x = pickle.dumps(x)
#     conn.sendall(pickle.dumps(len(send_x)))

#     resp_len = conn.recv(1024).decode() # get yes len msg

#     print("inter tensor half send")
...
#     print("begin recv")
#     data_len = pickle.loads(conn.recv(1024))
#     print("successful recv len")
#     conn.sendall("yes len".encode())
#     print("successful send check")
...
#     send_x = pickle.dumps(x)
#     conn.sendall(send_x)      
#     flag = pickle.loads(conn.recv(1024))    # get response
#     if flag == 'failed':   # 超时重传一次
#         conn.sendall(send_x)
#         print("msg resend")
...
#         print(f'get_data: send failed msg')
#         data = pickle.loads(conn.recv(1024))
#     return data
...
#     print(flag)  
#     flag = flag.decode()
#     # flag = conn.recv(40).decode()     # get response
#     if flag == 'failed':   # 超时重传一次
#         conn.sendall(send_x)
#         print("msg resend")
...
def send_short_data(conn, x, msg="msg", show=True):
    # 将消息长度编码为4字节的大端字节序
    send_x = pickle.dumps(x)
    length_bytes = len(send_x).to_bytes(4, byteorder='big')
    # 发送消息长度
