...
def cached_infer(sub_models, model_type, global_cache, client_caches, dataLoaders, device, cache_size):
    """ 返回平均推理时延 和 准确率 列表"""
    # 初始化
    ...
    for _ in range(warm_up_epoch):
        ...
        _ = cached_forward(sub_models, model_type, client_caches[0], global_avg_pooling, dummy_input, cache_size)
            ...
            if iter_sign:
                try:
                    ...
                    for x, y in zip(data, labels):
                        ...
                        x = x.unsqueeze(0)
                        start_time = time.perf_counter()
                        hit_idx, hit, res = cached_forward(sub_models, model_type, client_caches[cnum], global_avg_pooling, x, cache_size)
                        end_time = time.perf_counter()
...
if __name__ == "__main__":
    ...
    # 多客户端缓存推理
    update = False
    avg_time_list, correct_list, sample_num_list, correct_ratio_list = cached_infer(sub_models, model_type, global_cache, client_caches, dataLoaders, device, cache_size)
